...
(
    ...
    { // If true, the ray does not intersect with the volume
...
#pragma unroll 3 //Suggests the compiler unroll the loop for efficiency
        for (int j = 0; j < 3; ++j) { //x , y, z로 각기 계산
            ...
            //Determine grid cell indices: ray.l[j] is the index of the current grid cell.
            ray.l[j] = min(static_cast<int32_t>(ray.pos[j]), grid.size[j] - 2); 
            // Compute the fractional part of the position within the cell 
            ray.pos[j] -= static_cast<float>(ray.l[j]);
        }

        const float skip = compute_skip_dist(ray, //Determine how far the ray can skip to the next cell
                       grid.links, grid.stride_x, // based on its direction and current cell size
                       grid.size[2], 0);

        if (skip >= opt.step_size) {
            // For consistency, we skip the by step size
            //If the skip distance is larger than the step size, adjust t and continue to the next iteration of the loop
            t += ceilf(skip / opt.step_size) * opt.step_size;
            continue;
        }
        float sigma = trilerp_cuvol_one( //Compute density: Use trilinear interpolation to get the density
                ...
                ray.l, ray.pos,
                0);
        if (opt.last_sample_opaque && t + opt.step_size > ray.tmax) //Handle opaque last sample: If the option specifies, 
        {                                           //adjust the ray’s world step size for the final sample if it's close to the end.
            ray.world_step = 1e9;
        }
        // if (opt.randomize && opt.random_sigma_std > 0.0) sigma += ray.rng.randn() * opt.random_sigma_std;

        if (sigma > opt.sigma_thresh) { //Check if density exceeds threshold
            float lane_color = trilerp_cuvol_one( //Get the color (lane_color) at the current position 
                            grid.links,           //using spherical harmonic data.
                            grid.sh_data,
                            grid.stride_x,
                            grid.size[2],
                            grid.sh_data_dim,
            ...
            const float pcnt = ray.world_step * sigma; //Calculate percentage of light absorbed
            const float weight = _EXP(log_transmit) * (1.f - _EXP(-pcnt)); //he weight of the contribution 
                                                                             //to the output based on the transmission.
            ...
            if (_EXP(log_transmit) < opt.stop_thresh) //If the transmitted light falls below a threshold, exit the loop.
            {
                log_transmit = -1e3f;
                break;
            }
    ...
    if (lane_colorgrp_id == 0) { //Only one thread per color group should store the result to avoid conflicts.
        if (out_log_transmit != nullptr) { //save the logarithmic transmission.
            *out_log_transmit = log_transmit;
        }
        out[lane_colorgrp] = outv; //Save the computed color value to the output array.(output volume)
    }
}
