...
__device__ __inline__ float trilerp_cuvol_one( 
        ...
        const int idx) 
{
    const int32_t* __restrict__ link_ptr = links + (offx * l[0] + offy * l[1] + l[2]); //voxel 접근 offset계산
 //lerp(a,b,t)=a×(1−t)+b×t
 //MAYBE_READ_LINK(0) reads the value at the index specified by link_ptr[0]
 // pos[2]: This is a floating-point value representing the position within the voxel along the z-axis. 
//It is used as the interpolation factor t in the lerp function.
